#!/usr/bin/env python3
"""
Assemble `md_files/07_portfolio_construction.md` summarising the results of
every Problem 7 run, plus the written parts 7f (financing) and 7g (capacity).
Reads `results/07_summary.csv` and individual weight tables.
"""
...
VARIANTS = [
    ("a_unconstrained", "(a) unconstrained"),
    ("b1_1pct", "(b.i) 1% box"),
    ("b2_50bps", "(b.ii) 50 bps box"),
    ("b3_10pct", "(b.iii) 10% box"),
    ("c_f1_neutral", "(c) F1 neutral"),
    ("d_sector_neutral", "(d) sector neutral"),
    ("e1_all_neutral", "(e.i) factors + sectors neutral"),
    ("e2_shock_10bps", "(e.ii) ≤10 bps shock cap"),
]


def _fmt_exp(d: dict, prefix: str, *, digits: int = 4):
    parts = []
    for k, v in d.items():
        if k.startswith(prefix):
            parts.append(f"{k.replace(prefix, '')}:{v:+.{digits}e}")
    return ", ".join(parts)


def variant_stats_table() -> str:
    summary = pd.read_csv(RESULTS / "07_summary.csv")
    rows = []
    header = ("| variant | μᵀw | r̂ᵀw | σ_a | gross | n_long | n_short | "
              "max|w| | note |")
    rows.append(header)
    rows.append("|" + "|".join(["---"] * 9) + "|")
    for key, label in VARIANTS:
        r = summary[summary["variant"] == key]
        if r.empty:
            continue
        r = r.iloc[0]
        wdf = pd.read_csv(RESULTS / f"07_{key}" / "weights.csv")
        w = wdf["weight"].to_numpy()
        n_long = int((w > 1e-9).sum())
        n_short = int((w < -1e-9).sum())
        rows.append(
            f"| {label} | {r['expected_return']:+.3f} | {r['realized_return']:+.3f} | "
            f"{r['annual_vol']:.3%} | {r['gross']:.3f} | {n_long} | {n_short} | "
            f"{np.max(np.abs(w)):.3f} | {r['status']} |"
        )
    return "\n".join(rows)
...
def main() -> None:
    ...
    # Load all variant summaries/weights.
    narratives = []
    for key, label in VARIANTS:
        r = json.loads((RESULTS / f"07_{key}" / "summary.json").read_text())
        wdf = pd.read_csv(RESULTS / f"07_{key}" / "weights.csv")
        n_long = int((wdf["weight"] > 1e-9).sum())
        n_short = int((wdf["weight"] < -1e-9).sum())
        fac_exp = _fmt_exp(r, "factor_exp_factor_")
        sec_exp = _fmt_exp(r, "sector_exp_")
        narratives.append((key, label, r, n_long, n_short, fac_exp, sec_exp))
    ...
    md = [
        ...
        "",
        f"- n long/short: {narratives[0][3]}/{narratives[0][4]}.",
        f"- expected: {narratives[0][2]['expected_return']:+.3%} (monthly),",
        f"  realised: {narratives[0][2]['realized_return']:+.3%}.",
        f"- factor exposures: {narratives[0][5]}.",
        "",
    ...
    (MD / "07_portfolio_construction.md").write_text("\n".join(md), encoding="utf-8")
    print("wrote md_files/07_portfolio_construction.md")
